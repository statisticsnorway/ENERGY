...
*OUTPUT CLOSE ALL.
DATASET CLOSE ALL.
*OBS!!! change path in net line to actual computer.
...
*Comment per 17.06.2022: We are missing community iterviews for a total of 78 households/records and 4 EAs.
*04032011106001
*04032711103001
*08052011103003
*14060821104013.    
...
DATASET CLOSE ALL.
*OBS!!! change path in net line to actual computer.
*CD "S:\Organisasjon\A200\S216\D Betalte oppdrag\Energy\E Study visits\2022 06 Workshop NBS_INE_SSB_Oslo\2022 WS Oslo TZ".
...
********************************************************************.
*Save temporary HHQ + COM file and open again.
SAVE OUTFILE='tmp\TZHHCOM_1b.sav'
/KEEP 
ALL 
/COMPRESSED.
...
*Comment 17.06: Missing 24 records 
...
LIST REC_ID GeocodeHH HHExpAll mean_HHExpAll sd_HHExpAll n_cases.
EXECUTE.

*one outlier ( gt 3x stdv from mean) imputed as mean value..
IF (REC_ID = 1262) HHExpAll = mean_HHExpAll..
EXECUTE.

*check.
FREQUENCIES HHExpAll.

*check input to macro below.
FREQUENCIES UrbRur HHSize_gr1 higheduc_gr1.
***********************************************.

*B2) Hotdeck macro - nearest neighbour in strata imputation for missing HHExpAll.(higlight all between astrixes and run the HD).
...
**********************************
*check HD results.
FREQUENCIES HHExpAllHD.

IF (SYSMIS(HHExpAll)=1) HHExpAll = HHExpAllHD.
EXECUTE.
...
IF (REC_ID=1291) HHExpAll = mean_HHExpAll.
EXECUTE.

*check.
FREQUENCIES HHExpAll.
*comment 17.06 no missing or outlier on HHExpAll.
...
CTMP1 CTMP2 CTMP3 HHExpAllHD mean_HHExpAll sd_HHExpAll n_cases.
...
FREQUENCIES WEIGHT.
TEMPORARY.
SELECT IF SYSMIS(WEIGHT)=1.
LIST Rec_ID GeocodeHH weight.

*Comment 17.06 missing weights for 20 households in EA 07020421101006. 
*Discuss with NBS/Bjørn if we should delete this EA or find correct weights/total population data   .

*For now, weight = 1. To be modified after discussion.     
IF (GeocodeEA = "07020421101006") WEIGHT = (1 * 1.0027).
EXECUTE.
...
*Cut/Paste from Bjørn 19.06.2022.

*A). BASIC ENERGY VARIABLES.
...
*****************************
*B) CREATE ENERGY TIERS
*Cut/paste from word-doc 19.06.2022 Bjørn. . 
...
********************************************************
*C) COOKING TIERS.
* cut/paste from Bjørn word-doc 19.06.22 COOKING TIERS.
...
FREQ I10 CO1ATEXPFUEL CO1BTEXPSTOVE CO1CTEXPEMISSION CO1ATEXPFUEL CO1BTEXPSTOVE.
...
FREQ CO1DEXPKITFLOOR CO1DEXPKITCEILING CO1DEXPKITHEIGHT
...
FREQ CO1GTEXPVENT CO1ETEXPVENTSTR CO1FTEXPVENTLEV.
...
FREQ CO1HEXPCTIME CO1HTEXPCTIME.
...
FREQ CO1HTEXPCTIME CO1ITCOOKEXP.
...
FREQ CO1IEXPOSURE CO1BTEXPSTOVE CO1FTEXPVENTLEV CO1HTEXPCTIME.
...
FREQ CO3ACONVFUEL CO3BCONVSPREP CO3TCONV.
...
FREQ TMPCOFUELCOST / STATISTICS=MEDIAN.
TEMPORARY.
SELECT IF I25 EQ 1.
FREQ TMPCOFUELCOST / STATISTICS=MEDIAN.
TEMPORARY.
...
FREQ COTCOOKINGSOL CO1ITCOOKEXP CO2TEFFICIENCY CO3TCONV CO4TSAFETY CO5TAFFORD CO6TAVAIL.
...
FREQUENCIES CO1DEXPKITFLOOR CO1DEXPKITCEILING CO1DEXPKITHEIGHT CO1DEXPKITVOL CO1HEXPCTIME  CO1IEXPOSURE CO3ACONVFUEL CO3BCONVSPREP COCONSUM COFUELCOST CO5AFFORD CO6AVAIL. 
...
FREQUENCIES CO1ATEXPFUEL CO1BTEXPSTOVE CO1CTEXPEMISSION CO1DTEXPKITVOL CO1ETEXPVENTSTR CO1FTEXPVENTLEV CO1GTEXPVENT CO1HTEXPCTIME CO1ITCOOKEXP CO2TEFFICIENCY CO3TCONV CO4TSAFETY CO5TAFFORD CO6TAVAIL COTCOOKINGSOL.
